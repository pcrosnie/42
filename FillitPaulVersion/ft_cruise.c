...
/*   Updated: 2016/01/07 11:28:10 by pcrosnie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_check_for_range_max(int *tab, int *str, int n)
{
	...
	while (i < n * 2)
	{
		...
		while (j < 8)
		{
			if (tab[0] + tab[i] == str[0] + str[j] &&
					tab[1] + tab[i + 1] == str[1] + str[j + 1])
				return (0);
...
int		ft_check_sol(t_noeud *ptr)
{
	...
	n = g_info[1];
	verttab = ft_set_tab_vertical(n);
	vortab = ft_set_horizontal(n);
	while (ptr != NULL)
	{
		if (ft_check_for_range_max(verttab, ptr->coord_piece, n) == 0
				|| ft_check_for_range_max(vortab, ptr->coord_piece, n) == 0)
			return (0);
		ptr = ptr->prev;
	}
	return (1);
